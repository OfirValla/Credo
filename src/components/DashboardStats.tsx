...
export function DashboardStats() {
    ...
    if (currencies.length === 0) {
        return <div className="text-center text-muted-foreground py-10">{t('noData')}</div>;
    }
    ...
    return (
        <div className="space-y-12 mb-8">
            {currencies.map(currency => {
                ...
                const chartData = currencyData.portfolioStats.map(stat => ({
                    name: stat.name,
                    balance: Math.round(stat.totalBalance),
                    payment: Math.round(stat.monthlyPayment),
                    color: stat.color?.replace('bg-', '') || 'blue-500'
                }));

                return (
                    <div key={currency} className="space-y-6">
                        <div className="flex items-center gap-2 mb-4">
                            <div className="text-xl font-bold text-gradient capitalize">{t('currencySection', { currency })}</div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                                <Card gradient>
                                    ...
                                    <CardContent>
                                        <div className="text-2xl font-bold">{formatCurrency(currencyData.totalBalance, currency)}</div>
                            ...
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                                <Card gradient>
                                    ...
                                    <CardContent>
                                        <div className="text-2xl font-bold">{formatCurrency(currencyData.totalMonthlyPayment, currency)}</div>
                            ...
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                                <Card gradient>
                                    ...
                                    <CardContent>
                                        <div className="text-2xl font-bold">{currencyData.portfolioStats.length}</div>
                                ...
                                <Card gradient>
                                    ...
                                    <CardContent>
                                        <div className="text-2xl font-bold">
                                            {currencyData.portfolioStats.length > 0 ? formatCurrency(currencyData.totalMonthlyPayment / currencyData.portfolioStats.length, currency) : 0}
                                        </div>
                                            ...
                                            <PieChart>
                                                ...
                                                >
                                                    {chartData.map((_entry, index) => (
                                                        <Cell key={`cell-${index}`} fill={getColorHex(currencyData.portfolioStats[index].color || 'blue-500')} />
                                                    ))}
                                                </Pie>
                                                <Tooltip
                                                    formatter={(value: any) => {
